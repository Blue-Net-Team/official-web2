...
| tb_ai_conversation | created_at, last_active_at | 会话时间范围筛选与活跃度统计 |
| tb_ai_turn | created_at | 提问时间范围筛选与趋势统计 |
...
| related_url | VARCHAR(500) | 相关链接URL |
...
### 30. AI 会话表 (tb_ai_conversation)

AI 对话的会话容器。会话 ID 由 ai-service 生成并回传前端（服务端权威），主 API 服务只读。用于会话列表与分析面板的会话量趋势。

| 字段 | 类型 | 说明 |
|------|------|------|
| id | VARCHAR(64) | 主键，会话 ID，由 ai-service 生成 |
| created_at | DATETIME | 会话创建时间 |
| last_active_at | DATETIME | 最后活跃时间，每次新增提问时更新 |

---

### 31. AI 提问轨迹表 (tb_ai_turn)

每个用户提问的完整执行轨迹。由 ai-service（Python）写入，主 API 服务只读。表结构与 schema 由后端 Flyway 管理。

轨迹的唯一真相来源是 `events` 字段：读取层不抽派生列，需要按意图/动作筛选时直接从 JSONB 中提取（例如 `events->0->>'intent'`）。后续若聚合查询变慢，可用 PostgreSQL 生成列补充可索引字段，无需改动写入侧。

| 字段 | 类型 | 说明 |
|------|------|------|
| id | BIGINT | 主键，自增 |
| conversation_id | VARCHAR(64) | 所属会话 ID（应用层维护关系，无物理外键） |
| seq | INT | 会话内序号，从 1 开始递增 |
| user_input | TEXT | 用户原始提问文本，不含预披露阶段拼接的检索结果 |
| answer | TEXT | 最终答案，被拦截或断流时可能为空 |
| prompt | JSONB | 图执行结束时组装完成的完整消息列表快照；拒答与直接回复路径为 NULL |
| events | JSONB | 原始执行事件流（intent / pre_disclose / reasoning / tool_call / tool_result / content） |
| degraded | BOOLEAN | 是否为不完整记录（客户端中途断流或采集异常） |
| duration_ms | INT | 本轮提问从开始到结束的耗时（毫秒） |
| created_at | DATETIME | 记录创建时间 |

> 会话的「消息数」= 该会话下本表行数（只计用户侧提问，不计助手回复），读时聚合，不冗余存储。
...
| tb_ai_turn | idx_ai_turn_conversation_seq | conversation_id, seq | INDEX |
| tb_ai_turn | idx_ai_turn_created_at | created_at | INDEX |
| tb_ai_conversation | idx_ai_conversation_last_active_at | last_active_at | INDEX |
| tb_ai_conversation | idx_ai_conversation_created_at | created_at | INDEX |
