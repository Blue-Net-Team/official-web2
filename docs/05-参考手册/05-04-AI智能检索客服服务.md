...
# 对话轨迹采集（管理后台回看）
TBD_RAG_TRACE_ENABLED=true          # 采集开关；false 则完全不写轨迹
TBD_RAG_TRACE_DB_URI=               # 空 = 复用 PGVECTOR_URI（同一个 Postgres）
```

## 对话轨迹采集与管理后台

为回答“用户问了什么 / AI 检索了什么 / 为什么这么答”，AI 服务在每次流式对话时把完整执行轨迹落库，后端提供只读接口，前端管理后台供开发团队回看。

### 数据流

```
ai-service (Python)              api-service (Java)             前端
  chat_stream 事件流 ── tee ──▶ tb_ai_* 表 ── 只读 + RBAC ──▶ /admin/ai-traces 三个页面
  （数据在 Python 产生）        （Flyway 管 schema）  （权限在 Java）
```

- **写入是 best-effort**：落库失败只记 warn，绝不阻塞或中断对话
- **断流也落库**：客户端中途断开（用户关掉聊天面板）仍会落库已采集的事件并标记 `degraded`

### 数据模型

| 表 | 内容 |
|---|---|
| `tb_ai_conversation` | 会话容器：id / created_at / last_active_at |
| `tb_ai_turn` | 一行 = 一个用户提问的完整轨迹 |

`tb_ai_turn` 关键字段：

| 字段 | 说明 |
|---|---|
| `user_input` | 用户原话（预披露富化前的原始输入） |
| `events` (jsonb) | **轨迹唯一真相来源**：intent / pre_disclose / reasoning / tool_call / tool_result / content |
| `prompt` (jsonb) | 完整 prompt 快照（图执行结束时的消息列表）；拒答与直接回复路径为 NULL |
| `answer` | 最终答案 |
| `degraded` | 是否为不完整记录 |

### 采集的结构化事件

- `intent`：意图判定（`intent` / `confidence` / `action`）—— 替代原来的中文散文
- `pre_disclose`：预披露阶段的自动标签与标签命中
- `tool_call` / `tool_result`：携带 `round`（提问内递增轮次）与 `blocked`（轮次超限拦截）

### 会话标识

`conversation_id` 由服务端权威生成，并通过 SSE 首帧 `{"type":"conversation_id",...}` 下发。前端不再自行生成。

### 保留策略

**全量保留，不提供清理或归档**。容量实测可控（单轮检索型提问约 27 KB，万级约 270 MB），而短于一年的保留期会损断跨招新季的趋势对比。

### 后端接口（只读）

前缀 `/api/v1/admin/ai-traces`，全部 `@RequiresPermission`：

| 接口 | 权限 |
|---|---|
| `GET /conversations` | `ai-trace:list` |
| `GET /conversations/{id}` | `ai-trace:detail` |
| `GET /statistics` | `ai-trace:statistics` |
| `GET /gaps` | `ai-trace:gap` |

统计口径：趋势按会话聚合，分布（意图 / 动作 / 拒答原因 / 工具使用 / 兜底率）按提问聚合。
...
- **文本分片**: 基于 LLM 的语义主题分段
- **对话轨迹采集**: 自研 tee + Postgres 落库（`trace/` 模块）
