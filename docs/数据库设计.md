...
| direction | ENUM | 关联方向（可选，仅 type=DIRECTION 时使用） |
...
| `DIRECTION` | `direction` | 方向介绍 | 各技术方向介绍 | `direction` |
...
- `direction` 字段仅在 `type=DIRECTION` 时有效，用于关联具体技术方向
- `competition_id` 字段仅在 `type=COMPETITION` 时有效，用于关联具体竞赛

**示例数据**：
```
id=1, type=DIRECTION, description="计算机视觉方向", file_id=101, direction=COMPUTER_VISION
id=2, type=COMPETITION, description="2024年智能车竞赛", file_id=102, competition_id=5
id=3, type=LABORATORY, description="实验室环境", file_id=103
```
