...
│   │       ├── admin            # 管理端接口（Admin前缀）
│   │       ├── algorithm        # 算法判题相关接口
│   │       ├── assessment       # 考核相关接口（题目、答案、会话、统计、队伍、时间）
│   │       ├── auth             # 认证相关接口（登录、登出、密码重置）
│   │       ├── bugreport        # Bug报告相关接口
│   │       ├── college          # 学院相关接口
│   │       ├── competition      # 竞赛相关接口
│   │       ├── enrollment       # 报名相关接口
...
- **Admin Controller 统一放在 `v1/admin/` 包下**，不得与其他业务 Controller 混放在同一领域子包中。
...
**注意**：管理端控制器类名**必须**以 `Admin` 开头、以 `Controller` 结尾，统一放置在 `v1/admin/` 包下。

##### 异常处理约定

- Controller 层方法**禁止**自行 try-catch 并返回 `ResponseMessage.error()`。
- 所有业务异常统一交由 `@ControllerAdvice`（`GlobalExceptionHandler`）处理。
- 仅允许在需要返回非 200 HTTP 状态码的场景（如登录接口返回 401）中使用 try-catch + `ResponseEntity`。
...
| 5 | `Result` | `ResponseDTO` | `api/converter/{aggregate}/XxxResponseConverter.java` | `toDTO(result)` |

**核心规则：**
- Repository 接口只操作 `Entity`，不返回 `VO`
- `infrastructure/repository/converter/` 使用**显式字段映射**，禁止使用 `BeanUtils.copyProperties`
- API ResponseConverter 的 `toDTO` / `toDTOForUser` / `toDTOList` 等 public 方法**禁止**直接接收 `domain.model.entity` 包下的领域实体参数；入参必须是 Application Result、Domain VO 或 DTO
