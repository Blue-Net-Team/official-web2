# 后端开发手册

## 技术栈

| 技术 | 版本 | 说明 |
|------|------|------|
| Java | 21 | 编程语言 |
| Spring Boot | 3.5.10 | 后端框架 |
| Spring Security | - | 安全框架 |
| Spring Data Redis | - | 缓存支持 |
| Spring Validation | - | 参数校验 |
| Spring AOP | - | 切面编程 |
| JWT (jjwt) | 0.12.5 | 认证令牌 |
| MyBatis-Plus | 3.5.7 | ORM框架 |
| MinIO / 阿里云 OSS | - | 对象存储 |
| PostgreSQL | - | 关系数据库 |

## 项目结构（DDD分层架构）

```
src/backend/src/main/java/com/bluenet/web/
├── api/                          # API层（接口适配层）
│   ├── controller/               # 控制器
│   │   └── v1/
│   │       ├── achievement/      # 成就相关接口
│   │       │   └── AchievementController.java
│   │       ├── admin/            # 管理员接口
│   │       │   ├── AdminAchievementController.java
│   │       │   ├── AdminAssessmentJudgementController.java
│   │       │   ├── AdminAssessmentQuestionController.java
│   │       │   ├── AdminAssessmentStatisticsController.java
│   │       │   ├── AdminAssessmentTimeController.java
│   │       │   ├── AdminBugReportController.java
│   │       │   ├── AdminCollegeController.java
│   │       │   ├── AdminCompetitionController.java
│   │       │   ├── AdminEquipmentController.java
│   │       │   ├── AdminLearningPathController.java
│   │       │   ├── AdminMessageTemplateController.java
│   │       │   ├── AdminQrcodeController.java
│   │       │   ├── AdminVenueController.java
│   │       │   ├── AuditStatisticsController.java
│   │       │   ├── PermissionAdminController.java
│   │       │   └── RolePermissionAdminController.java
│   │       ├── college/          # 学院相关接口
│   │       │   └── CollegeController.java
│   │       ├── competition/      # 竞赛相关接口
│   │       │   └── CompetitionController.java
│   │       ├── enrollment/       # 报名相关接口
│   │       │   ├── AdminEnrollController.java
│   │       │   └── EnrollController.java
│   │       ├── file/             # 文件相关接口
│   │       │   ├── FileDownloadController.java
│   │       │   └── FileUploadController.java
│   │       ├── health/           # 健康检查接口
│   │       │   └── HealthController.java
│   │       ├── learningpath/     # 学习路径相关接口
│   │       │   └── LearningPathController.java
│   │       ├── member/           # 成员相关接口
│   │       │   └── MemberController.java
│   │       ├── qrcode/           # 二维码相关接口
│   │       │   └── QrcodeController.java
│   │       ├── user/             # 用户相关接口
│   │       │   ├── UserExperienceController.java
│   │       │   ├── UserInfoController.java
│   │       │   └── UserProfileController.java
│   │       ├── AlgorithmJudgeController.java
│   │       ├── AssessmentAnswerController.java
│   │       ├── AssessmentQuestionController.java
│   │       ├── AssessmentSessionController.java
│   │       ├── AssessmentStatisticsController.java
│   │       ├── AssessmentTeamController.java
│   │       ├── AssessmentTimeController.java
│   │       ├── AuthController.java
│   │       ├── BugReportController.java
│   │       ├── EquipmentController.java
│   │       ├── ResetPasswordController.java
│   │       └── VenueController.java
│   ├── converter/                # API层转换器（RequestDTO → Command）
│   │   ├── achievement/
│   │   ├── algorithm_judge/
│   │   ├── assessment_answer/
│   │   ├── assessment_judgement/
│   │   ├── assessment_question/
│   │   ├── assessment_session/
│   │   ├── assessment_statistics/
│   │   ├── assessment_team/
│   │   ├── assessment_time/
│   │   ├── audit/
│   │   ├── auditstatistics/
│   │   ├── auth/
│   │   ├── bugreport/
│   │   ├── college/
│   │   ├── competition/
│   │   ├── enroll/
│   │   ├── equipment/
│   │   ├── file/
│   │   ├── learningpath/
│   │   ├── member/
│   │   ├── permission/
│   │   ├── qrcode/
│   │   ├── resetpassword/
│   │   ├── rolepermission/
│   │   ├── userexperience/
│   │   ├── userinfo/
│   │   └── venue/
│   └── dto/                      # 数据传输对象
│       ├── achievement/
│       ├── algorithm_judge/
│       ├── assessment_answer/
│       ├── assessment_judgement/
│       ├── assessment_question/
│       ├── assessment_session/
│       ├── assessment_statistics/
│       ├── assessment_team/
│       ├── assessment_time/
│       ├── audit/
│       ├── auth/
│       ├── bugreport/
│       ├── college/
│       ├── competition/
│       ├── enrollment/
│       ├── equipment/
│       ├── experience/
│       ├── file/
│       ├── health/
│       ├── learningpath/
│       ├── member/
│       ├── permission/
│       ├── qrcode/
│       ├── user/
│       ├── venue/
│       ├── ApiResponseAdvice.java
│       ├── PageDTO.java
│       └── ResponseMessage.java
├── application/                  # 应用层（用例层）
│   ├── command/                  # 命令对象（写操作参数，按聚合分包）
│   │   ├── achievement/
│   │   ├── algorithm_judge/
│   │   ├── assessment_answer/
│   │   ├── assessment_judgement/
│   │   ├── assessment_question/
│   │   ├── assessment_session/
│   │   ├── assessment_team/
│   │   ├── assessment_time/
│   │   ├── audit/
│   │   ├── auditstatistics/
│   │   ├── auth/
│   │   ├── bugreport/
│   │   ├── college/
│   │   ├── competition/
│   │   ├── enroll/
│   │   ├── equipment/
│   │   ├── file/
│   │   ├── knowledge/
│   │   ├── learningpath/
│   │   ├── member/
│   │   ├── permission/
│   │   ├── qrcode/
│   │   ├── resetpassword/
│   │   ├── rolepermission/
│   │   ├── softwareresource/
│   │   ├── userexperience/
│   │   ├── userinfo/
│   │   ├── venue/
│   │   └── wpsform/
│   ├── query/                    # 查询参数（读操作参数，按聚合分包）
│   │   ├── adminuser/
│   │   ├── auditstatistics/
│   │   ├── bugreport/
│   │   ├── enroll/
│   │   ├── member/
│   │   └── permission/
│   ├── result/                   # 应用层结果对象（按聚合/领域分包）
│   │   ├── achievement/
│   │   ├── adminuser/
│   │   ├── algorithm_judge/
│   │   ├── assessment/
│   │   ├── audit/
│   │   ├── auth/
│   │   ├── bugreport/
│   │   ├── college/
│   │   ├── comment/
│   │   ├── common/
│   │   ├── competition/
│   │   ├── enroll/
│   │   ├── equipment/
│   │   ├── file/
│   │   ├── judge/
│   │   ├── knowledge/
│   │   ├── learningpath/
│   │   ├── member/
│   │   ├── permission/
│   │   ├── qrcode/
│   │   ├── resetpassword/
│   │   ├── rolepermission/
│   │   ├── softwareresource/
│   │   ├── team/
│   │   ├── user/
│   │   └── venue/
│   ├── message/                  # 消息相关
│   │   ├── template/
│   │   ├── MessageDispatcher.java
│   │   ├── MessageRequest.java
│   │   ├── MessageTemplateInfo.java
│   │   └── MessageTemplateRegistry.java
│   └── service/                  # 应用服务
│       ├── assessment/           # 考核相关服务（含 AccessGuard、PublicationService 等编排组件）
│       ├── auth/                 # 认证相关服务
│       │   ├── credential/
│       │   ├── provider/
│       │   ├── session/
│       │   └── strategy/
│       ├── impl/                 # 应用服务实现
│       └── 各种应用服务接口
├── domain/                       # 领域层（核心业务层）
│   ├── exception/                # 领域异常
│   │   ├── BadRequest.java
│   │   ├── DataConflict.java
│   │   ├── DataNotFound.java
│   │   ├── Forbidden.java
│   │   ├── GlobalException.java
│   │   ├── TooManyRequests.java
│   │   └── Unauthorized.java
│   ├── model/
│   │   ├── entity/               # 实体类
│   │   │   ├── Achievement.java
│   │   │   ├── AlgorithmJudgeCaseResult.java
│   │   │   ├── AlgorithmJudgeJob.java
│   │   │   ├── AssessmentAnswer.java
│   │   │   ├── AssessmentDecision.java
│   │   │   ├── AssessmentJudgement.java
│   │   │   ├── AssessmentQuestion.java
│   │   │   ├── AssessmentSession.java
│   │   │   ├── AssessmentTeam.java
│   │   │   ├── AssessmentTeamMember.java
│   │   │   ├── AssessmentTime.java
│   │   │   ├── Audit.java
│   │   │   ├── College.java
│   │   │   ├── Comment.java
│   │   │   ├── Competition.java
│   │   │   ├── DirectionLearningStep.java
│   │   │   ├── Enroll.java
│   │   │   ├── Equipment.java
│   │   │   ├── File.java
│   │   │   ├── Member.java
│   │   │   ├── MessageTemplate.java
│   │   │   ├── Permission.java
│   │   │   ├── Qrcode.java
│   │   │   ├── Role.java
│   │   │   ├── RolePermission.java
│   │   │   ├── User.java
│   │   │   ├── UserAchievement.java
│   │   │   ├── UserExperience.java
│   │   │   ├── Venue.java
│   │   │   └── VerifyCode.java
│   │   ├── enumerate/            # 枚举类
│   │   │   ├── AchievementType.java
│   │   │   ├── AlgorithmTestcaseType.java
│   │   │   ├── AuditStatisticsPeriod.java
│   │   │   ├── AwardLevel.java
│   │   │   ├── BugReportStatus.java
│   │   │   ├── Direction.java
│   │   │   ├── EnrollStatus.java
│   │   │   ├── ExperienceType.java
│   │   │   ├── FileType.java
│   │   │   ├── Gender.java
│   │   │   ├── JudgeCaseStatus.java
│   │   │   ├── JudgeJobStatus.java
│   │   │   ├── JudgementSource.java
│   │   │   ├── JudgementStatus.java
│   │   │   ├── LocalLoginType.java
│   │   │   ├── MessageChannel.java
│   │   │   ├── MessageContentType.java
│   │   │   ├── ObjectiveResultCode.java
│   │   │   ├── ProgrammingLanguage.java
│   │   │   ├── QrcodeType.java
│   │   │   ├── QuestionType.java
│   │   │   ├── ReviewerType.java
│   │   │   ├── RoleType.java
│   │   │   └── ValueEnum.java
│   │   ├── readmodel/            # 读模型（Repository 层投影/只读对象）
│   │   │   ├── AchievementReadModel.java
│   │   │   ├── AssessmentCandidateScoreRowReadModel.java
│   │   │   ├── AssessmentQuestionSubmissionHistoryReadModel.java
│   │   │   ├── AssessmentQuestionSubmissionReadModel.java
│   │   │   └── CompetitionReadModel.java
│   │   ├── result/               # 领域结果对象（DomainService 返回给应用层）
│   │   │   ├── ConfirmUploadResult.java
│   │   │   └── PresignedUploadResult.java
│   │   └── vo/                   # 值对象（无 *VO 后缀，按语义分包）
│   │       ├── experience_content/# UserExperience.content JSON 值对象
│   │       │   ├── CompetitionContent.java
│   │       │   ├── InternshipContent.java
│   │       │   └── ProjectContent.java
│   │       ├── question_content/ # AssessmentQuestion.content JSON 值对象
│   │       │   ├── AlgorithmContent.java
│   │       │   ├── FileUploadContent.java
│   │       │   ├── MultipleChoiceContent.java
│   │       │   ├── QuestionContent.java
│   │       │   └── SingleChoiceContent.java
│   │       ├── GitHubUserInfo.java
│   │       ├── OAuthState.java
│   │       ├── UserOnboardingCreateUserRequest.java
│   │       ├── UserOnboardingResult.java
│   │       └── wps/
│   │           └── WpsFormField.java
│   ├── repository/               # 仓储接口
│   │   ├── AchievementRepository.java
│   │   ├── AlgorithmJudgeCaseResultRepository.java
│   │   ├── AlgorithmJudgeJobRepository.java
│   │   ├── AssessmentAnswerRepository.java
│   │   ├── AssessmentDecisionRepository.java
│   │   ├── AssessmentJudgementRepository.java
│   │   ├── AssessmentQuestionRepository.java
│   │   ├── AssessmentSessionRepository.java
│   │   ├── AssessmentTeamRepository.java
│   │   ├── AssessmentTimeRepository.java
│   │   ├── BugReportRepository.java
│   │   ├── AuditRepository.java
│   │   ├── CollegeRepository.java
│   │   ├── CompetitionRepository.java
│   │   ├── EnrollRepository.java
│   │   ├── EquipmentRepository.java
│   │   ├── FileRepository.java
│   │   ├── LearningPathRepository.java
│   │   ├── MemberRepository.java
│   │   ├── PermissionRepository.java
│   │   ├── QrcodeRepository.java
│   │   ├── RolePermissionRepository.java
│   │   ├── RoleRepository.java
│   │   ├── UserExperienceRepository.java
│   │   └── UserRepository.java
│   └── service/                  # 领域服务（仅保留含实际业务逻辑的）
│       └── impl/
│           ├── AssessmentDecisionDomainServiceImpl.java
│           ├── AssessmentJudgementDomainServiceImpl.java
│           ├── AuthDomainServiceImpl.java
│           ├── EnrollDomainServiceImpl.java
│           ├── FileDomainServiceImpl.java
│           ├── QrcodeDomainService.java
│           ├── UserDomainServiceImpl.java
│           ├── UserExperienceDomainService.java
│           └── VerificationCodeDomainService.java
├── infrastructure/               # 基础设施层
│   ├── config/                   # 配置类
│   ├── repository/               # 仓储实现
│   │   ├── converter/            # 仓储转换器
│   │   ├── impl/                 # 仓储实现
│   │   └── mapper/               # MyBatis Mapper
│   └── security/                 # 安全相关
└── BluenetBackendApplication.java
```

## 功能模块

### 1. 认证模块

**登录方式**:
- 学号 + 密码登录
- 邮箱验证码登录
- GitHub OAuth登录

**认证流程**:
```mermaid
flowchart TD
    A[登录界面] --> B{选择登录方式}
    B --> C[学号登录]
    B --> D[邮箱登录]
    B --> E[GitHub OAuth]
    
    C --> F[验证学号密码]
    D --> G[发送验证码]
    E --> H[GitHub授权回调]
    
    F --> I[生成JWT]
    G --> J[验证验证码]
    H --> I
    
    J --> I
    I --> K[返回Token]
```

### 2. 报名模块

**报名流程**:
```mermaid
sequenceDiagram
    participant U as 用户
    participant F as 前端
    participant B as 后端

    Note over U,B: 步骤1: 上传头像（预签名直传）
    U->>F: 选择头像图片
    F->>B: POST /api/v1/file/prepare-upload (type=AVATAR)
    B-->>F: 返回 fileId + uploadUrl + callbackToken
    F->>OSS: PUT uploadUrl（直传MinIO）
    OSS-->>F: 上传成功
    F->>B: POST /api/v1/file/confirm-upload (fileId, callbackToken)
    B-->>F: 返回确认结果
    
    Note over U,B: 步骤2: 提交报名
    U->>F: 填写报名表单并提交
    F->>B: POST /api/v1/enroll
    B->>B: 查询学号是否已存在
    
    alt 学号不存在
        B->>B: 创建报名记录
        B-->>F: 返回成功
    else 学号已存在
        B-->>F: 返回冲突提示
        U->>F: 确认更新
        F->>B: POST /api/v1/enroll (forceUpdate=true)
        B->>B: 更新报名记录
        B-->>F: 返回成功
    end
```

**报名状态**:
| 状态 | 说明 |
|------|------|
| pending | 待审核 |
| approved | 已通过 |
| rejected | 已拒绝 |

### 3. 文件模块

#### 文件分类体系

系统采用两级分类体系管理文件：

**一级分类（FileType）**：文件的基本类型分类，对应对象存储中的 Key 前缀

| 枚举值 | 存储值 | 描述 | 对象 Key 前缀 | 权限控制 |
|--------|--------|------|--------------|----------|
| `AVATAR` | `avatar` | 用户头像 | `avatar/` | 团队成员可见或本人头像 |
| `NORMAL_IMG` | `normal-img` | 普通图片（包括场地、设备、竞赛等的图片） | `normal-img/` | 公开可见 |
| `ASSESSMENT_ATTACHMENT` | `assessment-attachment` | 考题附件 | `assessment-attachment/` | 同方向用户可见 |
| `WORK` | `work` | 考生作品文件 | `work/` | 本人或团队成员可见 |
| `QRCODE` | `qrcode` | 二维码图片 | `qrcode/` | 公开可见 |

**注意**：
1. 论文/专利成就的图片不再通过 `tb_introduce_image` 表管理，而是直接在 `tb_achievement` 表中通过 `file_id` 字段关联。

#### 文件存储结构

```
对象存储 bucket（storage.bucket）
├── avatar/                    # 头像对象前缀
│   └── avatar-{uuid}.jpg
├── normal-img/                # 普通图片对象前缀
│   └── normal_img-{uuid}.jpg
├── assessment-attachment/     # 考题附件对象前缀
│   └── assessment_attachment-{uuid}.pdf
├── work/                      # 考生作品对象前缀
│   └── work-{uuid}.zip
└── qrcode/                    # 二维码对象前缀
    └── qrcode-{uuid}.png
```

#### 文件命名规则

- **格式**：`{fileType}-{uuid}.{ext}`
- **示例**：`avatar-123e4567-e89b-12d3-a456-426614174000.jpg`
- **注意**：文件名中的 fileType 使用枚举名称的小写形式（如 `normal_img`），对象 Key 前缀使用 `FileType.getValue()`（如 `normal-img`）

#### 文件访问权限控制

文件访问权限根据关联的业务实体动态控制：

| 文件类型 | 权限控制表 | 判断逻辑 |
|----------|------------|----------|
| `work`（考生答案） | `tb_assessment_answer` | `currentUser.id == answer.user_id` OR `currentUser.role >= ROLE_MEMBER` |
| `assessment_attachment`（考题附件） | `tb_assessment_question` | `currentUser.direction == question.direction` |
| `avatar`（头像） | `tb_user` / `tb_enroll` | `currentUser.role >= ROLE_MEMBER` 或 `currentUser.avatarId == fileId` |
| `normal_img`（普通图片） | - | 公开可见 |
| `qrcode`（二维码） | - | 公开可见 |

**权限验证流程**:
```mermaid
flowchart TD
    A[请求文件] --> B{已登录?}
    B -->|否| C[拒绝访问]
    B -->|是| D{文件类型?}
    
    D -->|work| E{用户匹配或有权限?}
    E -->|是| F[允许访问]
    E -->|否| C
    
    D -->|assessment_attachment| G{方向匹配?}
    G -->|是| F
    G -->|否| C
    
    D -->|avatar| H{有角色或本人?}
    H -->|是| F
    H -->|否| C
    
    D -->|normal_img/qrcode| F
```

#### 文件上传接口（预签名直传）

系统采用三步预签名 URL 上传流程，文件直传 OSS（MinIO/阿里云），不经过后端：

| 步骤 | 接口 | 路径 | 说明 |
|------|------|------|------|
| 1 | 预签名上传准备 | `POST /api/v1/file/prepare-upload` | 生成预签名 PUT URL 和回调令牌 |
| 2 | 直传 OSS | `PUT {uploadUrl}` | 前端直接上传文件到对象存储 |
| 3 | 上传确认 | `POST /api/v1/file/confirm-upload` | 前端回调后端，校验文件并激活记录 |

**说明**：
- `prepare-upload` 时指定文件类型（fileType）、文件名、Content-Type、文件大小
- 返回 `fileId`、`uploadUrl`（预签名 PUT URL）、`callbackToken`
- 前端使用预签名 URL 直传 OSS 后，调用 `confirm-upload` 完成记录激活
- `AVATAR` 和 `NORMAL_IMG` 类型允许匿名上传（报名、Bug 报告场景），其他类型需要登录
- 旧的 `POST /api/v1/file/upload` 接口已废弃，不再推荐使用

#### 文件下载接口

- `GET /api/v1/file/download/{fileId}` - 统一下载接口，自动根据文件类型进行权限验证

### 4. 成员模块

**成员信息**:
- 基本信息（姓名、方向、职责）
- 微信二维码
- GitHub链接
- 项目经历
- 竞赛获奖
- 实习经历

**方向负责人判定**:
- `role = DIRECTION_ADMIN` 且 `direction` 字段对应方向

### 5. 考核模块

**考核结构**:
```
方向（计算机视觉/结构设计/嵌入式开发）
├── 轮次（一轮/二轮）
│   ├── 入学年份
│   │   ├── 题目列表
│   │   │   ├── 第一题
│   │   │   ├── 第二题
│   │   │   └── 第三题
```

**说明**：
- 后端使用入学年份替代传统的年级划分（大一/大二）
- 在报名审核时，可以根据入学年份控制用户参与考核的资格
- 这种方式可以更好地处理留级等特殊情况

**题目类型**:
| 类型 | 说明 |
|------|------|
| file_upload | 文件上传题 |
| algorithm | 算法题 |
| single_choice | 单选题 |
| multiple_choice | 多选题 |

**考核时间属性**:
| 属性 | 说明 |
|------|------|
| `timeLimit` | 是否限时考核 |
| `timeLimitMinutes` | 限时分钟数（限时考核必填） |
| `allowTeam` | 是否允许组队答题 |
| `resultsPublishedAt` | 考核结果发布时间，设置后考生可见评论和最终评分 |

**考核会话（AssessmentSession）**：
- 限时考核首次调用会自动创建会话
- 会话记录用户的考核开始时间和截止时间（deadline）
- 用于控制限时考核的答题时长

**考核答案（AssessmentAnswer）**：
- `teamId` 字段支持队伍答题，队伍成员提交答案时关联队伍 ID
- 算法题答案通过 `content` 存储源代码，`language` 存储编程语言

### 6. 考核队伍模块

**队伍管理**:
- 创建队伍：为指定考核时间创建队伍，当前用户自动成为队长，生成 6 位邀请码
- 加入队伍：通过邀请码加入队伍
- 离开队伍：成员可离开，队长不能离开（需先转让队长或解散）
- 转让队长：将队长权限转让给队伍中的另一名成员
- 解散队伍：仅队长可操作

**队伍约束**:
- 考核时间必须设置 `allowTeam = true` 才允许组队
- 已提交过个人答案的用户无法创建或加入队伍
- 考核结束后无法创建、加入队伍
- 队伍状态：`ACTIVE`（活跃）、`DISBANDED`（已解散）

### 7. 算法判题模块

**系统架构**：
- 基于 RabbitMQ 异步分发判题任务
- 交换机：`algorithm.judge`（DirectExchange）
- 三个队列：
  - `algorithm.judge.formal` — 正式判题队列
  - `algorithm.judge.run` — 自测/运行测试判题队列
  - `algorithm.judge.test-data` — 测试数据生成队列

**核心流程**：
1. **运行（Run）**：创建不计分运行任务，可使用默认运行用例或自定义输入
2. **提交（Submit）**：保存正式答案，创建判题任务并投递队列
3. **轮询（Poll）**：查询任务状态和用例结果

**判题任务状态**：
| 状态 | 说明 |
|------|------|
| PENDING | 待处理 |
| RUNNING | 运行中 |
| SUCCEEDED | 成功 |
| FAILED | 失败 |

**用例结果状态**：
| 状态 | 说明 |
|------|------|
| AC | 通过 |
| WA | 答案错误 |
| TLE | 超时 |
| MLE | 内存超限 |
| RE | 运行时错误 |
| CE | 编译错误 |

**权限控制**：
- 考生只能查看自己的判题任务
- 正式提交只向考生返回失败用例（AC 用例隐藏）
- 运行测试返回所有用例结果

### 8. Bug 报告模块

**功能概述**：
- 任何人可提交 Bug 报告（无需登录）
- 支持上传最多 3 张截图
- 可选同步到 GitHub Issue
- 管理员可查看、更新处理状态

**Bug 报告状态**：
| 状态 | 说明 |
|------|------|
| pending | 待处理 |
| in_progress | 处理中 |
| resolved | 已解决 |

**核心实体**：
- `BugReport`：Bug 报告聚合根，包含描述、页面 URL、环境信息、报告者邮箱、状态、GitHub Issue 信息
- `BugReportImage`：截图关联，通过 fileId 关联文件

**接口**：
- `POST /api/v1/bug-reports` — 提交 Bug 报告（公开）
- `GET /api/v1/admin/bug-reports` — 分页查询 Bug 报告列表（管理员）
- `GET /api/v1/admin/bug-reports/{id}` — 查看详情（管理员）
- `PUT /api/v1/admin/bug-reports/{id}/status` — 更新状态（管理员）

### 9. 竞赛模块

**竞赛管理**:
- 竞赛信息CRUD
- 竞赛图片管理
- 排序管理

### 10. 学习路径模块

**学习路径**:
- 按方向配置学习步骤
- 步骤排序管理

---

## 角色权限

### 角色层级

```mermaid
flowchart TD
    A[SUPER_ADMIN<br/>超级管理员] --> B[DIRECTION_ADMIN<br/>方向管理员]
    B --> C1[视觉负责人]
    B --> C2[结构负责人]
    B --> C3[电控负责人]
    B --> D[MEMBER<br/>团队成员]
    D --> E[CANDIDATE<br/>考核成员]
    E --> F[未登录]
```

### 权限说明

| 角色 | 权限范围 |
|------|----------|
| SUPER_ADMIN | 所有权限，可管理所有方向 |
| DIRECTION_ADMIN | 管理对应方向的考核时间、题目 |
| MEMBER | 查看报名、评论评分 |
| CANDIDATE | 参加考核、查看自己成绩 |

---

## API接口

详细接口文档请参考：[diagrams/接口总览.xmind](./diagrams/接口总览.xmind)

### 接口前缀

所有接口以 `/api/v1` 为前缀

### 主要接口模块

| 模块 | 路径 | 说明 |
|------|------|------|
| 认证 | /auth | 登录、登出 |
| 用户 | /user | 用户信息管理 |
| 消息 | /message | 验证码、消息模板 |
| 文件 | /file | 文件上传下载（预签名直传） |
| 介绍 | /intro | 竞赛、成就展示 |
| 报名 | /enroll | 报名管理 |
| 考核 | /assessment | 考核时间、题目、答案、评判、决策、会话、统计 |
| 考核队伍 | /assessment-teams | 组队答题管理 |
| 算法判题 | /algorithm-judge | 算法题运行、提交、轮询 |
| Bug报告 | /bug-reports | Bug提交与跟踪 |
| 审计 | /audit | 日志查询 |

---

## 开发规范

### 分层职责

| 层级 | 职责 |
|------|------|
| API层 | 接收请求、参数校验、返回响应 |
| 应用层 | 编排业务用例、对象转换、消息处理 |
| 领域层 | 核心业务逻辑、领域规则、实体管理 |
| 基础设施层 | 技术实现、外部依赖集成 |

### 命名规范

#### 包结构命名

```
com.bluenet.web
├── api                          # API层（接口适配层）
│   ├── controller               # 控制器
│   │   └── v1                   # 版本号
│   │       ├── achievement      # 成就相关接口
│   │       ├── admin            # 管理端接口（Admin前缀）
│   │       ├── algorithm        # 算法判题相关接口
│   │       ├── assessment       # 考核相关接口（题目、答案、会话、统计、队伍、时间）
│   │       ├── auth             # 认证相关接口（登录、登出、密码重置）
│   │       ├── bugreport        # Bug报告相关接口
│   │       ├── college          # 学院相关接口
│   │       ├── competition      # 竞赛相关接口
│   │       ├── enrollment       # 报名相关接口
│   │       ├── equipment        # 设备相关接口
│   │       ├── file             # 文件相关接口
│   │       ├── health           # 健康检查接口
│   │       ├── learningpath     # 学习路径相关接口
│   │       ├── member           # 成员相关接口
│   │       ├── qrcode           # 二维码相关接口
│   │       ├── user             # 用户相关接口
│   │       └── venue            # 场地相关接口
│   ├── converter                # API层转换器（RequestDTO → Command）
│   │   ├── achievement          # 成就相关转换器
│   │   ├── algorithm_judge      # 算法评测相关转换器
│   │   ├── assessment_answer    # 考核答案相关转换器
│   │   ├── assessment_judgement # 考核评判相关转换器
│   │   ├── assessment_question  # 考核题目相关转换器
│   │   ├── assessment_session   # 考核会话相关转换器
│   │   ├── assessment_statistics # 考核统计相关转换器
│   │   ├── assessment_team      # 考核队伍相关转换器
│   │   ├── assessment_time      # 考核时间相关转换器
│   │   ├── audit                # 审计相关转换器
│   │   ├── auditstatistics      # 审计统计相关转换器
│   │   ├── auth                 # 认证相关转换器
│   │   ├── college              # 学院相关转换器
│   │   ├── competition          # 竞赛相关转换器
│   │   ├── enroll               # 报名相关转换器
│   │   ├── equipment            # 设备相关转换器
│   │   ├── file                 # 文件相关转换器
│   │   ├── learningpath         # 学习路径相关转换器
│   │   ├── member               # 成员相关转换器
│   │   ├── permission           # 权限相关转换器
│   │   ├── qrcode               # 二维码相关转换器
│   │   ├── resetpassword        # 密码重置相关转换器
│   │   ├── rolepermission       # 角色权限相关转换器
│   │   ├── userexperience       # 用户经历相关转换器
│   │   ├── userinfo             # 用户信息相关转换器
│   │   └── venue                # 场地相关转换器
│   └── dto                      # 数据传输对象
│       ├── achievement          # 成就相关DTO
│       ├── algorithm_judge      # 算法评测相关DTO
│       ├── assessment_answer    # 考核答案相关DTO
│       ├── assessment_judgement # 考核评判相关DTO
│       ├── assessment_question  # 考核题目相关DTO
│       ├── assessment_session   # 考核会话相关DTO
│       ├── assessment_statistics # 考核统计相关DTO
│       ├── assessment_team      # 考核队伍相关DTO
│       ├── assessment_time      # 考核时间相关DTO
│       ├── audit                # 审计相关DTO
│       ├── auth                 # 认证相关DTO
│       ├── college              # 学院相关DTO
│       ├── competition          # 竞赛相关DTO
│       ├── enrollment           # 报名相关DTO
│       ├── equipment            # 设备相关DTO
│       ├── experience           # 经历相关DTO
│       ├── file                 # 文件相关DTO
│       ├── health               # 健康检查相关DTO
│       ├── learningpath         # 学习路径相关DTO
│       ├── member               # 成员相关DTO
│       ├── permission           # 权限相关DTO
│       ├── qrcode               # 二维码相关DTO
│       ├── user                 # 用户相关DTO
│       ├── venue                # 场地相关DTO
│       ├── ApiResponseAdvice.java
│       ├── PageDTO.java
│       └── ResponseMessage.java
├── application                  # 应用层（用例层）
│   ├── command                  # 命令对象（写操作参数，按聚合分包）
│   │   ├── achievement          # 成就相关命令
│   │   ├── algorithm_judge      # 算法评测相关命令
│   │   ├── assessment_answer    # 考核答案相关命令
│   │   ├── assessment_judgement # 考核评判相关命令
│   │   ├── assessment_question  # 考核题目相关命令
│   │   ├── assessment_session   # 考核会话相关命令
│   │   ├── assessment_team      # 考核队伍相关命令
│   │   ├── assessment_time      # 考核时间相关命令
│   │   ├── audit                # 审计相关命令
│   │   ├── auditstatistics      # 审计统计相关命令
│   │   ├── auth                 # 认证相关命令
│   │   ├── college              # 学院相关命令
│   │   ├── competition          # 竞赛相关命令
│   │   ├── enroll               # 报名相关命令
│   │   ├── equipment            # 设备相关命令
│   │   ├── file                 # 文件相关命令
│   │   ├── knowledge            # 知识库相关命令
│   │   ├── learningpath         # 学习路径相关命令
│   │   ├── member               # 成员相关命令
│   │   ├── permission           # 权限相关命令
│   │   ├── qrcode               # 二维码相关命令
│   │   ├── resetpassword        # 密码重置相关命令
│   │   ├── rolepermission       # 角色权限相关命令
│   │   ├── softwareresource     # 软件资源相关命令
│   │   ├── userexperience       # 用户经历相关命令
│   │   ├── userinfo             # 用户信息相关命令
│   │   ├── venue                # 场地相关命令
│   │   └── wpsform              # WPS 表单相关命令
│   ├── query                    # 查询参数（读操作参数，按聚合分包）
│   │   ├── adminuser            # 用户管理列表查询
│   │   ├── auditstatistics      # 审计统计查询
│   │   ├── bugreport            # Bug 报告列表查询
│   │   ├── enroll               # 报名列表查询
│   │   ├── member               # 成员列表查询
│   │   └── permission           # 权限列表查询
│   ├── result                   # 应用层结果对象（按聚合/领域分包）
│   │   ├── achievement
│   │   ├── adminuser
│   │   ├── algorithm_judge
│   │   ├── assessment
│   │   ├── audit
│   │   ├── auth
│   │   ├── bugreport
│   │   ├── college
│   │   ├── comment
│   │   ├── common
│   │   ├── competition
│   │   ├── enroll
│   │   ├── equipment
│   │   ├── file
│   │   ├── judge
│   │   ├── knowledge
│   │   ├── learningpath
│   │   ├── member
│   │   ├── permission
│   │   ├── qrcode
│   │   ├── resetpassword
│   │   ├── rolepermission
│   │   ├── softwareresource
│   │   ├── team
│   │   ├── user
│   │   └── venue
│   ├── message                  # 消息相关
│   │   ├── template             # 消息模板
│   │   ├── MessageDispatcher.java
│   │   ├── MessageRequest.java
│   │   ├── MessageTemplateInfo.java
│   │   └── MessageTemplateRegistry.java
│   ├── service                  # 应用服务
│   │   ├── assessment           # 考核相关服务（含 AccessGuard、PublicationService 等编排组件）
│   │   ├── auth                 # 认证相关服务
│   │   │   ├── credential       # 凭证相关
│   │   │   ├── provider         # 认证提供者
│   │   │   ├── session          # 会话管理
│   │   │   └── strategy         # 认证策略
│   │   ├── impl                 # 服务实现
│   │   └── 各种应用服务接口
│   ├── result                   # 应用层结果对象（按聚合/领域分包）
│   │   ├── achievement
│   │   ├── adminuser
│   │   ├── algorithm_judge
│   │   ├── assessment
│   │   ├── audit
│   │   ├── auth
│   │   ├── bugreport
│   │   ├── college
│   │   ├── comment
│   │   ├── common
│   │   ├── competition
│   │   ├── enroll
│   │   ├── equipment
│   │   ├── file
│   │   ├── judge
│   │   ├── knowledge
│   │   ├── learningpath
│   │   ├── member
│   │   ├── permission
│   │   ├── qrcode
│   │   ├── resetpassword
│   │   ├── rolepermission
│   │   ├── softwareresource
│   │   ├── team
│   │   ├── user
│   │   └── venue
├── domain                       # 领域层（核心业务层）
│   ├── exception                # 领域异常
│   ├── model
│   │   ├── entity               # 实体类
│   │   ├── enumerate            # 枚举类
│   │   ├── readmodel            # 读模型（Repository 层投影/只读对象）
│   │   ├── result               # 领域结果对象（DomainService 返回给应用层）
│   │   └── vo                   # 值对象（无 *VO 后缀，按语义分包）
│   │       ├── experience_content # UserExperience.content JSON 值对象
│   │       ├── question_content # AssessmentQuestion.content JSON 值对象
│   │       ├── GitHubUserInfo.java
│   │       ├── OAuthState.java
│   │       ├── UserOnboardingCreateUserRequest.java
│   │       ├── UserOnboardingResult.java
│   │       └── wps
│   │           └── WpsFormField.java
│   ├── repository               # 仓储接口
│   └── service                  # 领域服务
│       └── impl                 # 领域服务实现
├── infrastructure               # 基础设施层
│   ├── config                   # 配置类
│   ├── repository               # 仓储实现
│   │   ├── converter            # 仓储转换器
│   │   ├── impl                 # 仓储实现
│   │   └── mapper               # MyBatis Mapper
│   └── security                 # 安全相关
└── BluenetBackendApplication.java
```

#### Controller 层规范

##### 包结构约定

- **所有 Controller 必须按领域/聚合根放入 `v1/` 下的子包中**，`v1` 根包禁止直接存放任何 Controller 文件。
- **Admin Controller 统一放在 `v1/admin/` 包下**，不得与其他业务 Controller 混放在同一领域子包中。
- 子包命名使用小写，多个单词直接拼接（如 `learningpath`、`bugreport`）。

##### 类命名约定

| 类型 | 命名规则 | 示例 |
|------|----------|------|
| 控制器 | `XxxController.java` | `MemberController.java` |
| 管理端控制器 | `AdminXxxController.java` | `AdminEnrollController.java` |

**注意**：管理端控制器类名**必须**以 `Admin` 开头、以 `Controller` 结尾，统一放置在 `v1/admin/` 包下。

##### 异常处理约定

- Controller 层方法**禁止**自行 try-catch 并返回 `ResponseMessage.error()`。
- 所有业务异常统一交由 `@ControllerAdvice`（`GlobalExceptionHandler`）处理。
- 仅允许在需要返回非 200 HTTP 状态码的场景（如登录接口返回 401）中使用 try-catch + `ResponseEntity`。

**✅ 正确示例**：
```java
@GetMapping("/{id}")
public ResponseMessage<MemberDTO> getMember(@PathVariable Long id) {
    MemberResult result = memberAppService.getMemberById(id);
    return ResponseMessage.success(memberResponseConverter.toDTO(result));
}
```

**❌ 错误示例**：
```java
@GetMapping("/{id}")
public ResponseMessage<MemberDTO> getMember(@PathVariable Long id) {
    try {
        MemberResult result = memberAppService.getMemberById(id);
        return ResponseMessage.success(memberResponseConverter.toDTO(result));
    } catch (DataNotFound e) {
        return ResponseMessage.error(404, e.getMessage());  // 不要这样做
    }
}
```

#### 类命名规范

| 类型 | 命名规则 | 示例 |
|------|----------|------|
| 控制器 | `XxxController.java` | `MemberController.java` |
| 管理端控制器 | `AdminXxxController.java` | `AdminEnrollController.java` |
| 应用服务接口 | `XxxAppService.java` | `MemberAppService.java` |
| 应用服务实现 | `XxxAppServiceImpl.java` | `MemberAppServiceImpl.java` |
| 领域服务实现 | `XxxDomainServiceImpl.java` | `EnrollDomainServiceImpl.java` |
| 仓储接口 | `XxxRepository.java` | `UserRepository.java` |
| API请求转换器 | `XxxRequestConverter.java` | `MemberRequestConverter.java` |
| API响应转换器 | `XxxResponseConverter.java` | `MemberResponseConverter.java` |
| 命令对象 | `XxxCommands.java` | `MemberCommands.java` |
| 查询参数 | `XxxQuery.java` / `GetXxxQuery.java` | `GetMemberListQuery.java` |
| 应用层结果对象 | `XxxResult.java` | `MemberResult.java` |
| 领域结果对象 | `XxxResult.java`（位于 `domain.model.result`） | `ConfirmUploadResult.java` |
| 读模型 | `XxxReadModel.java` | `AchievementReadModel.java` |
| Mapper | `XxxMapper.java` | `UserMapper.java` |
| 实体类 | 实体名（PascalCase） | `User.java`、`UserExperience.java` |
| 枚举类 | 实体名（PascalCase） | `Direction.java`、`EnrollStatus.java` |
| 值对象 | 语义化名称（**禁止**使用 `*VO` 后缀） | `QuestionContent.java`、`OAuthState.java` |
| 配置类 | `XxxConfig.java` / `XxxProperties.java` | `SecurityConfig.java`、`JwtProperties.java` |
| 异常类 | 描述性名词 | `BadRequest.java`、`DataNotFound.java` |

**已废弃**：`*VO.java` 命名方式。历史原因遗留的 `*VO` 后缀值对象已全部删除，新增值对象禁止使用 `VO` 后缀。

#### DTO命名规范

| 类型 | 命名规则 | 示例 |
|------|----------|------|
| 请求DTO | `{动作}{实体}RequestDTO.java` | `CreateEnrollmentRequestDTO.java` |
| 响应DTO | `{实体}ResponseDTO.java` 或 `{实体}DTO.java` | `UserAuthResponseDTO.java`、`MemberBriefDTO.java` |
| 列表查询DTO | `{实体}ListQueryDTO.java` | `EnrollmentListQueryDTO.java` |
| 详情DTO | `{实体}DetailDTO.java` | `MemberDetailDTO.java` |
| 简要DTO | `{实体}BriefDTO.java` | `MemberBriefDTO.java` |
| 统计DTO | `{实体}StatisticsDTO.java` | `EnrollmentStatisticsDTO.java` |
| 分页DTO | `PageDTO.java` | `PageDTO<T>` |
| 通用响应 | `ResponseMessage.java` | `ResponseMessage<T>` |

#### 方法命名规范

| 操作类型 | 方法前缀 | 示例 |
|----------|----------|------|
| 查询单个 | `get` / `find` | `getMemberById()`、`findByUsername()` |
| 查询列表 | `list` / `getAll` | `listMembers()`、`getAllColleges()` |
| 分页查询 | `page` / `getPaged` | `pageEnrollments()` |
| 创建 | `create` | `createEnrollment()` |
| 更新 | `update` | `updateProfile()` |
| 删除 | `delete` | `deleteCompetition()` |
| 验证 | `verify` | `verifyEmail()` |
| 提交 | `submit` | `submitAnswer()` |
| 审核 | `approve` / `reject` | `approveEnrollment()`、`rejectEnrollment()` |
| 统计 | `statistics` / `count` | `getEnrollmentStatistics()` |
| 删除 | `delete` / `remove` | `deleteExperience()` |
| 判断存在 | `exists` | `existsByUsername()` |
| 统计 | `count` | `countByStatus()` |
| 校验 | `validate` / `check` | `validatePermission()` |
| 转换 | `to{目标类型}` | `toDTO()`、`toVO()`、`toEntity()` |

#### 类型转换链（不可违反）

各层之间的数据流转必须遵循以下转换链：

```
RequestDTO → Command → Entity → Result → ResponseDTO
                  ↑       ↑
              Query   ReadModel
```

| 转换步骤 | 源类型 | 目标类型 | 转换器位置 | 示例 |
|----------|--------|----------|-----------|------|
| 1a | `RequestDTO` | `Command` | `api/converter/{aggregate}/XxxRequestConverter.java` | `toCommand(dto)` |
| 1b | `RequestDTO` / QueryString | `Query` | `api/converter/{aggregate}/XxxRequestConverter.java` 或 Controller 直接构造 | `toQuery(dto)` |
| 2 | `Command` | `Entity` | `AppServiceImpl` 通过工厂方法 | `Entity.create(command.field())` |
| 3 | `DO` ↔ `Entity` | `Entity` ↔ `DO` | `infrastructure/repository/converter/XxxRepositoryConverter.java` | `toEntity(do)` / `toDataObject(entity)` |
| 4a | `Entity` | `Application Result` | `AppServiceImpl` 直接构造 | `new Result(entity.getId(), ...)` |
| 4b | `ReadModel` | `Application Result` | `AppServiceImpl` 直接透传或简单构造 | 见统计/列表场景 |
| 4c | `Entity` | `Domain Result` | `DomainServiceImpl` 直接构造 | `new ConfirmUploadResult(fileId, ...)` |
| 5 | `Result` | `ResponseDTO` | `api/converter/{aggregate}/XxxResponseConverter.java` | `toDTO(result)` |

**核心规则：**
- Repository 接口优先返回 `Entity`；为性能需要在 SQL 层做投影时，可返回 `ReadModel`（必须在方法注释中说明）
- Repository 接口**禁止**返回 `*VO`、Application `*Result` 等上层对象
- `infrastructure/repository/converter/` 使用**显式字段映射**，禁止使用 `BeanUtils.copyProperties`
- API ResponseConverter 的 `toDTO` / `toDTOForUser` / `toDTOList` 等 public 方法**禁止**直接接收 `domain.model.entity` 包下的领域实体参数；入参必须是 Application Result、Domain Result、ReadModel、Domain VO 或 DTO
- 已删除的遗留工具：`RepositoryObjectConverter`（基于反射的通用转换器）

#### Entity 行为化规范

Entity 必须遵循以下模式：

```java
@Data
@NoArgsConstructor(access = AccessLevel.PRIVATE)
public class College {
    private Long id;
    private String name;

    private College(Long id, String name) {
        this.id = id;
        this.name = name;
    }

    /** 构造时校验 */
    public static College create(String name) {
        if (name == null || name.isBlank()) throw new IllegalArgumentException("...");
        return new College(null, name.trim());
    }

    /** 从数据库重建，跳过校验 */
    public static College reconstruct(Long id, String name) {
        return new College(id, name);
    }

    /** 领域行为 */
    public void rename(String newName) {
        if (newName == null || newName.isBlank()) throw new IllegalArgumentException("...");
        this.name = newName.trim();
    }
}
```

**规范要点：**
- 无参构造必须为 `PRIVATE`
- 提供 `create(...)` 工厂方法（构造时校验）
- 提供 `reconstruct(...)` 工厂方法（从数据库重建，跳过校验）
- 领域行为方法直接修改状态（如 `rename`、`updateInfo`）
- 禁止在 Entity 中使用 `Builder`（会破坏构造私有化）

#### DomainService 保留标准

重构过程中删除了大量纯代理型 DomainService，仅保留含**实际业务逻辑**的 DomainService：

| 保留的 DomainService | 保留原因 |
|---------------------|---------|
| `AssessmentAnswerDomainService` | 答案提交/更新规则（方向/年级匹配、时间窗口、淘汰判断、重复提交、文件校验、团队同步、客观题自动评判触发） |
| `AssessmentDecisionDomainService` | 考生最终通过决策算法、跨轮次淘汰规则 |
| `AssessmentJudgementDomainService` | 判题逻辑、评分计算 |
| `AssessmentTeamDomainService` | 组队生命周期规则（创建/加入/退出/转让/解散约束） |
| `AuthDomainService` | 本地登录校验 |
| `FileDomainService` | 文件存储、关联查询、下载权限校验 |
| `QrcodeDomainService` | 二维码生成、级联删除 |
| `VerificationCodeDomainService` | 验证码生成与校验 |

**已删除的 DomainService：**
- `UserDomainService`：用户状态变更已下沉到 `User` Entity
- `UserExperienceDomainService`：经历 CRUD 直接由 `UserExperienceAppService` 调用 Repository
- `CommentDomainService`：评论权限与去重已下沉到 `Comment` Entity
- `EnrollDomainService`：报名审批流程已重构到应用层

**删除标准：**
- 仅做参数透传、直接调用 Repository 的纯代理型 DomainService → **删除**
- 业务逻辑已下沉到 Entity 行为方法 → **删除**
- 含跨聚合协调、复杂算法、外部服务调用的 → **保留**

#### 变量命名规范

| 类型 | 命名规则 | 示例 |
|------|----------|------|
| 类成员变量 | camelCase | `memberRoleId`、`candidateRoleId` |
| 常量 | UPPER_SNAKE_CASE | `MAX_PAGE_SIZE`、`DEFAULT_SORT_ORDER` |
| 布尔变量 | `is`/`has`/`can` 前缀 | `isActive`、`hasPermission`、`canEdit` |
| 集合变量 | 复数形式 | `members`、`enrollmentList` |
| Map变量 | `xxxMap` 或 `xxxByKey` | `permissionMap`、`usersById` |

#### 数据库命名规范

| 类型 | 命名规则 | 示例 |
|------|----------|------|
| 表名 | `tb_` + 小写下划线 | `tb_user`、`tb_user_experience` |
| 字段名 | 小写下划线 | `student_id`、`created_at` |
| 索引名 | `idx_` + 表名 + 字段 | `idx_user_student_id` |
| 外键名 | `fk_` + 表名 + 关联表 | `fk_user_role` |

### 枚举规范

#### 枚举定义

枚举类使用 `@EnumValue` 注解标记存储值，采用**小写下划线**格式：

```java
@Getter
public enum Direction {
    COMPUTER_VISION("computer_vision", "计算机视觉"),
    STRUCTURAL_DESIGN("structural_design", "结构设计"),
    EMBEDDED("embedded", "嵌入式开发");

    @EnumValue  // 标记存储到数据库的值
    private final String value;
    private final String description;

    Direction(String value, String description) {
        this.value = value;
        this.description = description;
    }
}
```

#### 枚举格式对照

| 场景 | 格式 | 示例 |
|------|------|------|
| Java代码引用 | 大写下划线 | `Direction.COMPUTER_VISION` |
| 数据库存储 | 小写下划线 | `computer_vision` |
| API请求参数 | 小写下划线 | `?direction=computer_vision` |
| API响应JSON | 小写下划线 | `"direction": "computer_vision"` |

#### 枚举转换机制

项目使用 `EnumConverterFactory` 处理字符串到枚举的转换：

1. **优先匹配 `@EnumValue` 值**：`computer_vision` → `COMPUTER_VISION`
2. 其次尝试大写匹配：`COMPUTER_VISION` → `COMPUTER_VISION`
3. 最后不区分大小写匹配

#### API参数示例

```bash
# 成员列表按方向筛选
GET /api/v1/members?direction=computer_vision

# 报名列表按状态筛选
GET /api/v1/admin/enrollments?status=pending
```

### 测试规范

#### 角色名称定义

系统角色名称统一使用**大写下划线**格式：

| 角色名 | 说明 |
|--------|------|
| SUPER_ADMIN | 超级管理员 |
| DIRECTION_ADMIN | 方向管理员 |
| MEMBER | 团队成员 |
| CANDIDATE | 考生（报名通过后自动分配） |

#### 测试中设置用户角色

由于数据库迁移脚本中插入的角色**没有指定ID**（使用数据库自增），测试时不能直接通过ID设置用户角色，必须先通过角色名查找角色ID：

```java
@SpringBootTest
class ExampleIntegrationTest {
    
    @Autowired
    private RoleMapper roleMapper;
    
    private Long memberRoleId;
    private Long candidateRoleId;
    
    @BeforeEach
    void setUp() {
        // 正确：通过角色名查找角色ID
        Role memberRole = roleMapper.selectByName("MEMBER");
        assertNotNull(memberRole, "MEMBER 角色应存在");
        memberRoleId = memberRole.getId();
        
        Role candidateRole = roleMapper.selectByName("CANDIDATE");
        assertNotNull(candidateRole, "CANDIDATE 角色应存在");
        candidateRoleId = candidateRole.getId();
        
        // 然后使用查找到的ID创建用户
        User user = User.create(
            "2024001001",           // studentId
            "test@example.com",     // email
            memberRoleId,           // roleId
            encodedPassword,        // password
            "测试用户",              // username
            null, null, null, null, null, null, null, null, null, null, null, null
        );
    }
}
```

#### 错误示例

```java
// 错误：不要硬编码角色ID，且禁止使用 Builder 模式创建领域实体
User user = User.create(
    "2024001001", "test@example.com", 1L, encodedPassword,
    "测试用户", null, null, null, null, null, null, null, null, null, null, null, null, null
);  // ❌ 错误：ID可能因数据库状态不同而变化
```

#### 使用 @WithSecurityPrincipal 注解

单元测试可使用 `@WithSecurityPrincipal` 注解模拟已认证用户：

```java
@Test
@WithSecurityPrincipal(
    userId = 1L,
    studentId = "2024001001",
    username = "张三",
    roleType = RoleType.MEMBER,
    permissions = "user:info:me"
)
void testUserInfo() {
    // 测试逻辑
}
```

`@WithSecurityPrincipal` 会构造一个 `SecurityPrincipal` 放入 `UserCTX`，与安全上下文实际运行时保持一致。

### 异常处理

使用统一异常处理：
- `BadRequest`: 400 错误请求
- `Unauthorized`: 401 未授权
- `Forbidden`: 403 禁止访问
- `DataNotFound`: 404 数据不存在
- `DataConflict`: 409 数据冲突

### 分页接口规范

**必须使用 `PageDTO<T>` 而非 Spring Data 的 `Page<T>`**

Spring Data 的 `Page<T>` 序列化后的字段与前端 `PageDTO<T>` 接口定义不匹配，所有分页接口必须返回 `PageDTO<T>`。

#### Controller 层示例

```java
// ✅ 正确
@GetMapping
public ResponseMessage<PageDTO<AchievementDTO>> getAchievements(
        @RequestParam(defaultValue = "0") Integer page,
        @RequestParam(defaultValue = "12") Integer size) {
    return ResponseMessage.success(achievementService.getAchievements(page, size, ...));
}

// ❌ 错误 - Spring Data Page 序列化格式与前端不兼容
@GetMapping
public ResponseMessage<Page<AchievementDTO>> getAchievements(...) {
    return ResponseMessage.success(achievementService.getAchievements(...));
}
```

#### Service 层示例

```java
@Override
public PageDTO<AchievementDTO> getAchievements(Integer page, Integer size, ...) {
    // 1. 创建分页请求
    Pageable pageable = PageRequest.of(page, size);
    
    // 2. 执行分页查询
    Page<AchievementReadModel> readModelPage = repository.findWithFilter(..., pageable);
    
    // 3. 转换为 DTO 分页
    Page<AchievementDTO> dtoPage = readModelPage.map(converter::convertToDTO);
    
    // 4. 关键：转换为 PageDTO
    return PageDTO.from(dtoPage);
}
```

#### 前端对应类型

```typescript
// 前端 PageDTO 接口定义
interface PageDTO<T> {
  content: T[]           // 内容列表
  totalElements: number  // 总元素数
  totalPages: number     // 总页数
  number: number         // 当前页码(从0开始)
  size: number           // 每页大小
  numberOfElements: number  // 当前页元素数
  first: boolean         // 是否第一页
  last: boolean          // 是否最后一页
  empty: boolean         // 是否为空
}
```

#### 注意事项

1. Controller 返回类型必须是 `ResponseMessage<PageDTO<XxxDTO>>`
2. Service 层使用 `PageDTO.from(dtoPage)` 进行转换
3. 分页参数：`page` 从 0 开始，`size` 默认值根据业务场景设置

### 审计日志

所有操作记录审计日志：
- 操作人、操作时间
- 请求参数（JSON格式）
- 客户端IP、User-Agent
- 操作结果

### 数据库规范

#### 表设计规范

- 所有表以 `tb_` 为前缀
- **不使用**软删除字段，数据直接物理删除
- **不需要** `created_at`、`updated_at`、`created_by`、`updated_by` 等审计字段，除非有特殊业务需求
- 使用 `tb_audit` 表统一记录操作审计日志

#### 迁移脚本规范

- **不要**在数据库迁移脚本中添加权限的默认值
- 权限数据应通过代码初始化或管理后台配置
- 迁移脚本只负责表结构定义，不包含业务初始数据

#### 特殊审计字段场景

以下表因业务需要保留审计字段：

| 表名 | 审计字段 | 说明 |
|------|----------|------|
| tb_competition | created_at, updated_at | 竞赛创建和更新时间 |
| tb_comment | created_at, updated_at | 评论时间记录 |
| tb_audit | action_time | 审计日志操作时间 |
| tb_user_experience | start_time, end_time | 经历时间段 |

---

## 层间返回类型约定（项目规范）

目的：统一 Mapper / Repository / Domain / Application / Controller 的返回值，减少责任混淆与隐性耦合。

### 总体原则

- 不在下层直接暴露上层类型（如 DTO）。
- Mapper 负责数据库行和 XML 查询结果映射。RepositoryImpl 负责 DO / QueryDO -> Domain Entity / Domain VO 的转换。Application 层负责 Domain -> DTO 的转换。Controller 统一返回 ResponseMessage<DTO>。
- SQL 逻辑必须写在 Mapper XML 中；Java 代码中不得通过 `QueryWrapper.apply()`、`.last()`、注解 SQL 或字符串片段承载 SQL 逻辑。

### Mapper 返回值类型

Mapper 只能返回以下三类类型，且返回对象 nullable，不使用 Optional。

1. 普通 DO
   - 适用场景：查询结果能自然映射为某张表的数据对象。
   - 典型场景：按主键查单表、按唯一字段查单表、单表分页/列表查询、单表增删改。
   - 示例：`UserDO selectById(Long id);`、`IPage<AssessmentQuestionDO> selectPageByAssessmentTimeId(...);`

2. QueryDO
   - 适用场景：查询结果不是某个单表实体的完整或自然形态，而是为某个 XML 查询场景组合出的持久化查询数据对象。
   - 典型场景：多表 join 混合字段、group by / count / sum / avg 聚合、窗口函数、排名、去重后的派生字段、报表或工作台查询结果。
   - 命名：`XxxQueryDO`。
   - 包路径：`com.bluenet.web.infrastructure.repository.dataobject.query`。
   - 限制：QueryDO 只能出现在 `infrastructure.repository.mapper`、`infrastructure.repository.impl` 和 `infrastructure.repository.dataobject.query` 内部，不允许出现在 domain repository 接口、domain service、application service 或 controller。
   - 示例：`AssessmentQuestionScoreboardQueryDO`、`AssessmentCandidateScoreQueryDO`、`AuditTrendPointQueryDO`。

3. 基础类型
   - 适用场景：查询结果只是单个标量或简单集合。
   - 典型场景：count / exists、id 集合、name/value 集合、布尔判断。
   - 示例：`long countByName(...)`、`boolean existsByRoleIdAndPermissionId(...)`、`List<Long> selectPermissionIdsByRoleId(...)`。

禁止事项：
- Mapper 禁止返回 API DTO、Application DTO、Domain VO。
- Mapper 禁止返回命名为 `Projection`、`View`、`DTO` 的查询结果类型。
- Mapper 禁止通过注解 SQL 承载查询；SQL 应写在 XML 中。
- Java 代码禁止写 SQL 片段或 SQL 函数表达式；复杂条件应下沉到 XML。

- Repository 接口（domain/repository）
  - 返回：Optional<Aggregate>、Aggregate、List<Aggregate>、Page<Aggregate>、int（受影响行数）等。
  - 示例：Optional<User> findById(Long id);
  - 说明：接口不得暴露持久层 DO、QueryDO、Mapper、IPage、MyBatis 类型。

- RepositoryImpl（infrastructure/impl）
  - 职责：调用 Mapper（获得 DO 或 QueryDO），转换为 Domain Entity/Aggregate/VO，处理空值并返回 Optional。保持转换逻辑集中。
  - 示例：public Optional<User> findById(Long id) { UserDO d = userMapper.selectById(id); return Optional.ofNullable(convert(d)); }

- 领域服务（domain/service）
  - 返回：Domain Entity/Aggregate 或领域 VO（仅用于值对象如统计）。不得返回 DTO。
  - 示例：Optional<User> getUser(Long id); TabCountsVO getTabCounts(Long userId);

- Application 层 & Controller
  - Application 返回 DTO（或 PageDTO），Controller 返回 ResponseMessage<DTO>。
  - 示例：public UserInfo getMyInfo();  Controller: public ResponseMessage<UserInfo> getMyInfo()

### Optional 与空值处理约定

- Mapper 返回 nullable 对象；RepositoryImpl 负责将 null 转为 Optional（或其它统一的空值策略）。不要在 Mapper 方法签名中使用 Optional。

### 分页约定

- Mapper 返回原始分页数据（IPage/分页 DO 或 QueryDO），RepositoryImpl 转换为 Page<Aggregate/VO>，Application 转为 PageDTO<DTO>。


## 配置说明

### 环境配置

- `dev`: 开发环境（默认）
- `test`: 测试环境
- `prod`: 生产环境

### 启动命令

```bash
# 开发环境
mvn spring-boot:run

# 指定环境
mvn spring-boot:run -Dspring.profiles.active=prod

# 打包
mvn clean package -Pprod
```
