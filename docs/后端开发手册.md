...
2. 实验室环境和设备信息现在分别通过 `tb_venue` 和 `tb_equipment` 表管理，不再使用 ImageType 枚举进行分类。
...
使用通用的文件上传接口，上传文件后获得 fileId，然后在创建对应表记录时使用该 fileId 进行关联：

| 接口 | 路径 | 权限 |
|------|------|------|
| 通用文件上传 | `POST /api/v1/file/upload` | 需登录 |

**说明**：
- 上传时需指定文件类型（fileType）
- 上传成功后返回 fileId
- 然后在创建场地、设备、竞赛等记录时，将 fileId 关联到对应字段
