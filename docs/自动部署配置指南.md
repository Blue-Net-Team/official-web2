...
| `DEPLOY_KEY` | SSH 私钥（完整内容） | `-----BEGIN OPENSSH PRIVATE KEY-----...` |
...
#### 生产环境 SSH 凭据（master 分支）

| Secret 名称 | 说明 | 示例 |
|-------------|------|------|
| `DEPLOY_USER_PROD` | 生产环境 SSH 用户名 | `root` 或 `deploy` |
| `DEPLOY_KEY_PROD` | 生产环境 SSH 私钥 | `-----BEGIN OPENSSH PRIVATE KEY-----...` |
...
| `DEPLOY_KEY_DEV` | 开发环境 SSH 私钥 | `-----BEGIN OPENSSH PRIVATE KEY-----...` |
...
|------|------|----------|----------|----------|
| `master` | 生产环境 | `latest` | `DEPLOY_PATH_PROD` | `DEPLOY_HOST_PROD` |
...
| `build-and-push` | api-service-test, frontend-test | 仅 master push |
| `deploy` | build-and-push | 仅 master push |
...
- name: Build frontend image
  ...
  with:
    build-args: |
      ...
      NEXT_PUBLIC_AI_SERVICE_HOST=${{ steps.vars.outputs.deploy_host }}
      NEXT_PUBLIC_AI_SERVICE_PORT=8000
      NEXT_PUBLIC_AI_SERVICE_SSL_ENABLED=false
      NEXT_PUBLIC_AI_SERVICE_PREFIX=/ai/v1
```
