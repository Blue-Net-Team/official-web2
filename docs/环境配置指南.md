...
应用启动时会自动确保 `STORAGE_BUCKET` 配置的单一存储桶存在。不同文件类型通过对象 Key 前缀区分：
...
| `STORAGE_BUCKET` | `bluenet` | 对象存储单一 bucket 名称 |
...
# ===========================================
# 用于自动将 Bug 报告同步到 GitHub Issue
# 若不配置或留空，则同步逻辑自动跳过，不影响正常提交
...
MSYS_NO_PATHCONV=1 docker run -d \
  ...
  --network bluenet_bluenet-network \
  -p 8080:8080 \
