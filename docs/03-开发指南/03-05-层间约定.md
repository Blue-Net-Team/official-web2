...
- 不在下层直接暴露上层类型（如 DTO）。
- Mapper 负责数据库行和 XML 查询结果映射。RepositoryImpl 负责 DO / QueryDO -> Domain Entity / ReadModel 的转换。Application 层负责 Domain -> DTO 的转换。Controller 统一返回 `ResponseMessage<DTO>`。
...
- Mapper 禁止返回 API DTO、Application DTO、Domain Result。
...
- 职责：调用 Mapper（获得 DO 或 QueryDO），转换为 Domain Entity / Aggregate / ReadModel，处理空值并返回 Optional。保持转换逻辑集中。
- 示例：
...
- 返回：Domain Entity / Aggregate 或领域结果对象（如 `ConfirmUploadResult`）。不得返回 DTO。
- 示例：`Optional<User> findById(Long id);`、`ConfirmUploadResult confirmUpload(...);`
...
- Mapper 返回原始分页数据（IPage/分页 DO 或 QueryDO），RepositoryImpl 转换为 `Page<Aggregate / ReadModel>`，Application 转为 `PageDTO<DTO>`。
