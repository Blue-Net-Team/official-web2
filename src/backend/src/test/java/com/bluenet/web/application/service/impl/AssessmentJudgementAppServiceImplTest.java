...
class AssessmentJudgementAppServiceImplTest {
    ...
    void reviewFileUploadAnswer_memberAndFileUpload_shouldCreateManualJudgement() {
        try (MockedStatic<UserCTX> mockedUserCTX = mockStatic(UserCTX.class)) {
            ...
            AssessmentJudgementResult result = assessmentJudgementAppService.reviewFileUploadAnswer(
                    new AssessmentJudgementCommands.ManualReviewCommand(ANSWER_ID, BigDecimal.valueOf(8), "完成度较好"));

            assertEquals(JudgementSource.MANUAL, result.source());
            assertEquals(ReviewerType.MEMBER, result.reviewerType());
    ...
    void reviewFileUploadAnswer_candidate_shouldThrowForbidden() {
        try (MockedStatic<UserCTX> mockedUserCTX = mockStatic(UserCTX.class)) {
            ...
            assertThrows(
                    Forbidden.class,
                    () -> assessmentJudgementAppService.reviewFileUploadAnswer(
                            new AssessmentJudgementCommands.ManualReviewCommand(ANSWER_ID, BigDecimal.ONE, null)));
    ...
    void reviewFileUploadAnswer_scoreAboveMax_shouldThrowBadRequest() {
        try (MockedStatic<UserCTX> mockedUserCTX = mockStatic(UserCTX.class)) {
            ...
            assertThrows(
                    BadRequest.class,
                    () -> assessmentJudgementAppService.reviewFileUploadAnswer(
                            new AssessmentJudgementCommands.ManualReviewCommand(ANSWER_ID, BigDecimal.valueOf(11),
                                    null)));
        }
    }
