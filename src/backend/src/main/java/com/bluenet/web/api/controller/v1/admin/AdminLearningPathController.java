...
public class AdminLearningPathController {
    ...
    @Operation(summary = "创建学习步骤", description = "为指定方向创建新的学习步骤，新步骤追加到该方向末尾")
    @ApiResponses({
            ...
            @ApiResponse(responseCode = "400", description = "参数错误", content = @Content(mediaType = "application/json", schema = @Schema(implementation = ResponseMessage.class), examples = @ExampleObject(value = "{\"code\":400,\"msg\":\"步骤标题不能为空\",\"data\":null}"))),
    ...
    @Operation(summary = "更新学习步骤", description = "更新指定学习步骤的信息，不改变其展示顺序")
    @ApiResponses({
    ...
    @Operation(summary = "调整学习步骤排序", description = "批量覆盖指定方向学习步骤的展示顺序，数值越小越靠前")
    @ApiResponses({
            ...
            @ApiResponse(responseCode = "400", description = "排序项不属于该方向或步骤不存在", content = @Content(mediaType = "application/json", schema = @Schema(implementation = ResponseMessage.class), examples = @ExampleObject(value = "{\"code\":400,\"msg\":\"学习步骤不存在或不属于该方向: 99\",\"data\":null}"))),
            @ApiResponse(responseCode = "404", description = "方向标识无效", content = @Content(mediaType = "application/json", schema = @Schema(implementation = ResponseMessage.class), examples = @ExampleObject(value = "{\"code\":404,\"msg\":\"无效的方向标识: invalid\",\"data\":null}")))
    })
    @RequiresPermission(name = "调整学习步骤排序", value = "direction-learning-path:sort", access = AccessLevel.PROTECTED)
    @PutMapping("/{slug}/learning-steps/sort")
    public ResponseMessage<Void> batchUpdateSortOrder(
            @Parameter(description = "方向标识（cv/embed/struct）", required = true, example = "cv") @PathVariable String slug,
            @Valid @RequestBody BatchSortRequestDTO request) {
        learningPathAppService.batchUpdateSortOrder(learningPathRequestConverter.toCommand(slug, request));
        return ResponseMessage.success(null);
    }
}
