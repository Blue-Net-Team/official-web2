...
- `software_resource_lookup` 接受名称**列表**并内部 fan-out 查询，同时对名称做本地容错匹配（精确 → 别名 → 子串 → 相似度），解决 `keil uvision5` / `SolidWorks` / `vscode` 等写法差异。
- 明确软件类问题的两条分支流程：
...
- **测试**：`src/ai-service/tests/tools/test_software_resource_search.py` 重写；新增/调整 `tests/agent/` 下的流程与轮次上限测试。
