...
**新增**: 对于 `CANDIDATE` 角色，上述结果 SHALL 再排除被后续淘汰限制命中的考核：如果该用户在相同 `direction+grade` 组合的某个更早 epoch 中被淘汰（`tb_assessment_decision.passed = false`），则该用户 SHALL NOT 看到 epoch 更高（或 epoch=0）的考核时间。
...
#### Scenario: Eliminated CANDIDATE cannot see next round
- **WHEN** CANDIDATE 用户在 epoch=1 的考核中被淘汰（`passed = false`）
- **AND** 系统存在 epoch=2 的考核（相同 direction+grade）
- **THEN** 该用户请求考核列表时 SHALL NOT 看到 epoch=2 的考核
- **AND** SHALL still see epoch=1 (the round they were eliminated from)

#### Scenario: Eliminated CANDIDATE cannot see final round
- **WHEN** CANDIDATE 用户在 epoch=1 的考核中被淘汰（`passed = false`）
- **AND** 系统存在 epoch=0（最终考核）的考核（相同 direction+grade）
- **THEN** 该用户请求考核列表时 SHALL NOT 看到 epoch=0 的考核
