...
Competition management providing CRUD operations, sorting management, logo upload, and photo association for competitions displayed on the website homepage and competition list.
...
### Requirement: Upload competition image endpoint
The system SHALL provide an endpoint to upload competition images with file and competitionId in a single request.

#### Scenario: Upload competition image
...
#### Scenario: Upload competition image to non-existent competition
...
#### Scenario: Upload competition image exceeding limit
- **WHEN** admin requests POST /api/v1/file/upload/competition/image when competition already has 20 images
- **THEN** system SHALL return 400 error with message "每个竞赛最多关联20张照片"

### Requirement: Upload competition logo endpoint
The system SHALL provide an endpoint to upload competition logo with file and competitionId in a single request.

#### Scenario: Upload competition logo
...
#### Scenario: Upload competition logo to non-existent competition
- **WHEN** admin requests POST /api/v1/file/upload/competition/logo with non-existent competitionId
- **THEN** system SHALL return 404 error with message "竞赛不存在"

### Requirement: Upload competition permission
The competition upload endpoints SHALL require admin permission.

#### Scenario: Upload competition image without authentication
- **WHEN** unauthenticated user requests POST /api/v1/file/upload/competition/image
- **THEN** system SHALL return 401 error

#### Scenario: Upload competition logo without authentication
- **WHEN** unauthenticated user requests POST /api/v1/file/upload/competition/logo
- **THEN** system SHALL return 401 error

#### Scenario: Upload competition image without admin role
- **WHEN** non-admin user requests POST /api/v1/file/upload/competition/image
- **THEN** system SHALL return 403 error

#### Scenario: Upload competition logo without admin role
- **WHEN** non-admin user requests POST /api/v1/file/upload/competition/logo
- **THEN** system SHALL return 403 error
