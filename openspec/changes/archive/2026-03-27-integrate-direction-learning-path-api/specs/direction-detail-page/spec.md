...
### Requirement: Direction learning path API service

The system SHALL provide an API service layer to fetch learning path data from backend.

#### Scenario: Successful API call
- **WHEN** service calls `getLearningPath('cv')`
- **THEN** system returns learning path data with video links

#### Scenario: API call failure
- **WHEN** backend API is unavailable
- **THEN** service throws error for caller to handle
