...
**新增**: When publishing decisions, the system SHALL ensure that any `passed = false` decision immediately activates the 7-day elimination restriction for that candidate. This SHALL happen automatically as part of the existing decision save logic (no additional publish-specific action required).
...
#### Scenario: Eliminated candidate immediately restricted after decision
- **WHEN** a direction administrator sets `passed = false` for a candidate
- **THEN** the candidate SHALL be immediately restricted from accessing subsequent assessment rounds
- **AND** the 7-day grace period SHALL begin from `decided_at`
