...
**选择**：AntD Table `expandable` + 队伍头行 + 默认展开。

**理由**：
- 队伍头行显示队伍名称、队长、人数、队伍总分/人均分/待评汇总
- 展开后内嵌 Table 显示组员个人评分详情
- 独立考生（未组队）作为普通行直接显示
- 默认全部展开，减少管理员操作

**队伍行数据模型**：
```
{ type: 'team', teamId, teamName, leaderName, memberCount, members[], key }
{ type: 'member' | 'independent', ...AssessmentCandidateScoreboardDTO, key }
```

**交互**：
- 队伍行点击：展开/折叠
- 组员行点击：打开评分明细抽屉（复用现有逻辑）
- 独立考生行点击：打开评分明细抽屉

### 8. 题目视图队伍标识

**选择**：在"考生"列中增加 Tag 显示队伍名和队长标记，SQL 已按队伍排序使同队相邻。

**样式**：队长显示 `👑队名`（蓝色 Tag），组员显示 `队名`（默认 Tag）。

### 9. 录用决策页面队伍标识

**选择**：决策列表不折叠（操作粒度 per-user），仅在考生列增加队伍 Tag；右侧详情面板增加"队伍"信息项。
...
- [SQL JOIN] 评分查询增加 LEFT JOIN 队伍表，对性能影响小（`tb_assessment_team_member` 有 `(team_id, user_id)` 唯一索引）。

## Migration Plan

1. 发布后端代码（自动创建组员 answer、锁定、清理逻辑、评分查询队伍字段）
2. 发布前端代码（全局选项、组队声明、队伍聚合展示）
