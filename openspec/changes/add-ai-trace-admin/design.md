...
**决定**：不提供任何自动删除、过期清理或归档能力。轨迹一旦落库就永久保留。

**理由**：

- 容量上完全不需要清理：实测单轮检索型提问约 **27 KB**（事件 15.5 KB + prompt 12 KB），
  万级提问约 **270 MB**，远低于单库承受范围。
- 本后台的目标是**产品分析与知识库反哺**，而招新是**年度周期**。
  任何短于一年的保留期都会让跨招新季的趋势对比直接断掉，反而损害核心用途。
- 数据量小 + 用途需要长历史 ⇒ 清理的收益为负。

**放弃的替代方案**：

- *180 天 TTL*：初稿曾选此方案，动机是"学生原话不应无限期留档"的隐私考虑。
  但权衡后认为它牺牲了核心用途（跨年对比）却换不来实质收益（存储本就无压力）。
  **该方案已废弃，相关代码、配置与测试已移除。**
- *归档到冷存储*：引入额外复杂度（对象存储、回迁、查询分叉），与总量 270 MB 的规模不匹配。

**若日后确需清理**：应作为独立变更重新评估，而不是在本功能里保留一个默认开启的删除任务。
