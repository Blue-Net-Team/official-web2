...
- **[prompt 快照体积]** 约 15–20 KB/提问，其中系统提示（`TAG_RETRIEVAL_SYSTEM_PROMPT`）每次完全相同、固定占用约 2 KB → 实现时先实测增长速率。若确有必要，可改为"系统提示只存一次 + 存内容 hash 引用"，但会牺牲快照保真度，**默认不做**。

- **[AI 服务与后端共享 Postgres，写放大]** 落库可能影响既有查询 → 缓解：单次批量插入（一个提问一次事务）、best-effort 且异常只 warn、必要时降级为关闭采集。已有 `tb_rag_*` 共库先例可参考。
...
- **[权限标识全局唯一]** `PermissionScanner` 遇到重复标识会直接让应用启动失败 → 已确认 `ai-trace:*` 四个标识与现有权限无冲突。
...
3. **chunk 正文截断是否在本次一并处理** —— `[:300]` / `[:200]` 的截断让 LLM 只看到 chunk 的前一部分，而 rerank 是按**完整正文**打分的，导致"排序依据被截掉"。这是一个独立于后台的 RAG 质量问题；本次只让它可观测。是否另开变更处理？

4. **截断长度不一致** —— 目前存在三处不同的截断值：`chunk_search_by_tags` 的正文 300、`registry._format_rerank_results` 的正文 200、`tag_description` 的 60（`registry.py` 与 `graph.py` 各一处）。是否统一为一个可配置值？
