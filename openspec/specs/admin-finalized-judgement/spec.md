...
- **WHEN** a direction administrator submits a final score and optional comment for a file upload answer
- **THEN** the system SHALL create or update the judgement record for that answer with source `ADMIN_FINALIZED`, the administrator's user id as reviewer, the provided score, and the provided comment
