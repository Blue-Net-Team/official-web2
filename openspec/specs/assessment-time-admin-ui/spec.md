...
### Requirement: 新增考核时间
系统 SHALL 提供「新增考核时间」按钮，点击后打开 Drawer 表单，包含字段：方向（Select）、轮次（InputNumber）、年级（InputNumber）、开始时间（DatePicker）、结束时间（DatePicker）、限时开关（Switch）、限时分钟数（InputNumber，限时开启时显示）。
...
### Requirement: 角色权限控制（前端）
前端 SHALL 根据用户角色控制操作按钮的可见性：
- SUPER_ADMIN：所有方向的新增、编辑、删除按钮均可用
- DIRECTION_ADMIN：新增时方向选择器限制为自己方向；其他方向记录的编辑/删除按钮不显示
...
### Requirement: 表格数据展示格式
表格 SHALL 对以下字段进行格式化展示：
- 方向：显示中文标签（如"计算机视觉"），使用 Tag 组件
